...
class FullGaussian_3d_sample_grid(Readout):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if gauss_type == "full":
            self.sigma_shape = (1, outdims, 3, 3)
        elif gauss_type == "uncorrelated":
            self.sigma_shape = (1, outdims, 1, 2)
        elif gauss_type == "isotropic":
            self.sigma_shape = (1, outdims, 1, 1)
    ...
    def init_grid_predictor(self, source_grid, hidden_features=20, hidden_layers=0, final_tanh=False):
        self._original_grid = False
        layers = [nn.Linear(source_grid.shape[1], hidden_features if hidden_layers > 0 else 3)]

        for i in range(hidden_layers):
            layers.extend(
                [
                    nn.ELU(),
                    nn.Linear(hidden_features, hidden_features if i < hidden_layers - 1 else 3),
                ]
            )
...
class FullGaussian2d_learnable_z(Readout):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        z = Parameter(torch.Tensor(outdims))
        self.register_parameter("z", z)
...
class FullGaussian2d_learnable_z_tanh(Readout):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.z_raw = Parameter(torch.Tensor(outdims))
        self.register_parameter("z_raw", self.z_raw)
        torch.nn.init.uniform_(self.z_raw, -0.1, 0.1)
...
class FullGaussian2d_Gumbel_softmax(Readout):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.z_logits = Parameter(
            torch.randn(outdims, in_shape[0])
    ...
    def forward(self, x, sample=None, shift=None, out_idx=None, **kwargs):
        ...
        bias = self.bias
        outdims = self.outdims
        z = F.gumbel_softmax(self.z_logits, tau=1.0, hard=True)
        z_int = torch.argmax(z, dim=-1)
...
class FullGaussian2d_Gumbel_softmax_learnable_tau(Readout):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.log_tau = Parameter(torch.log(torch.tensor(10.0)))
    ...
    def forward(self, x, sample=None, shift=None, out_idx=None, **kwargs):
        ...
        z = F.gumbel_softmax(self.z_logits, tau=tau, hard=True)
        z_int = torch.argmax(z, dim=-1)
...
class FullGaussian2d_Gumbel_softmax_scheduled_tau(Readout):
    ...
    def __repr__(self):
        c, w, h = self.in_shape
        r = "Sparse Readout. Gumbel-Softmax with scheduled temperature (tau).\n"
        r += self.gauss_type + " "
        r += self.__class__.__name__ + " (" + "{} x {} x {}".format(c, w, h) + " -> " + str(self.outdims) + ")"
        if self.bias is not None:
            r += " with bias"
...
class FullGaussian2d_REINFORCE(Readout):
    """
    Sparse Readout using REINFORCE algorithm to sample single channel per neuron.
    ...
    def forward(self, x, sample=None, shift=None, out_idx=None, **kwargs):
        ...
        z_probs = F.softmax(self.z_logits, dim=1)
        if self.training:
            z = torch.multinomial(z_probs, 1).squeeze(-1)
        else:
            z = z_probs.argmax(dim=1)

        self.z = z
        z_expanded = z.view(1, 1, outdims, 1).expand(N, 1, outdims, 1)
        ...
        y = F.grid_sample(x, grid, align_corners=self.align_corners)
        y = torch.gather(y, dim=1, index=z_expanded).view(N, outdims)
        y = y * feat.squeeze()
        ...
        # For REINFORCE loss
        log_probs = F.log_softmax(self.z_logits, dim=1)
        self.last_selected_log_probs = log_probs.gather(1, z.unsqueeze(1)).squeeze(1)

        return y

    def __repr__(self):
        c, w, h = self.in_shape
        r = "Sparse Readout. REINFORCE\n"
        r += self.gauss_type + " "
        r += self.__class__.__name__ + " (" + "{} x {} x {}".format(c, w, h) + " -> " + str(self.outdims) + ")"
        if self.bias is not None:
            r += " with bias"
...
class FullGaussian2d_adaptive_reg(Readout):
    """
    Copied from sinzlab/neuralpredictors:main
    A readout using a spatial transformer layer whose positions are sampled from one Gaussian per neuron. Mean
    and covariance of that Gaussian are learned.
